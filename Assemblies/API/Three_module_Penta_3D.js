...
import { modelFile, partsData } from "./Penta_3D_DB.js";
import { explosionSteps } from "./Penta_3D_Position.js"; 
    ...
    slider.addEventListener("input", (e) => {
        ...
        explosionSteps.forEach((step, index) => {
            ...
            itensPasso.forEach(item => {
                ...
                const localDirectionItem = ehObjeto && item.localDirection ? item.localDirection : step.localDirection;
                const distanceItem = ehObjeto && item.distance !== undefined ? item.distance : step.distance;
                ...
                if (mesh3d && posOriginal) {
                    ...
                    if (localDirectionItem) {
                        ...
                        const dirMundo = dirLocal.applyQuaternion(
                            mesh3d.getWorldQuaternion(new THREE.Quaternion())
                        );

                        mesh3d.position.addScaledVector(dirMundo, deslocamento);

                    } else if (step.axis === "x") {
                        mesh3d.position.x += deslocamento;
                    } else if (step.axis === "y") {
                        mesh3d.position.y += deslocamento;
                    } else if (step.axis === "z") {
                        mesh3d.position.z += deslocamento;
                    }
                }
            });
        });
...
if (btnInfoEl) {
    btnInfoEl.addEventListener("mouseenter", () => {
        ...
        partInfoImg.src = "./Files/Imagens/Info_Assemblies_Penta.png";
        pdfAtivo = ""; 
        partInfo.style.display = "block";
    });
