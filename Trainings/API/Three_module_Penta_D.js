...
function setTrainingImage(src, onLoadComplete){
    ...
    // Se está usando slider, transição instantânea; senão, 150ms
...
function processTargetStep() {
    ...
    // Avança um passo se ainda precisa ir pra frente
    if (currentStep < targetStep) {
        executeStep();
    }
    // Retrocede um passo se ainda precisa ir pra trás
...
if(slider3d){
    ...
    slider3d.addEventListener("input", (e) => {
        ...
        // Mapeamento: 0% = -1 (startScreen), 100% = totalSteps - 1 (último passo)
        targetStep = 
            Math.round(
