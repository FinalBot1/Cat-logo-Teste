...
function loadModel(path = "Assets/GLB/286-14x14x90.glb") {
  ...
  loader.load(path, (gltf) => {
    ...
    // 🔹 Ajuste somente para modelo Explode
   if (path.includes("Explode")) {
    model.position.y -= size.y * 0.2; // ajuste fino aqui
}
...
renderer.domElement.addEventListener("click", (event) => {
  ...
  lastClickedMeshes = [clickedObject];
});
